...
// --- CSS ---------------------------------------------------------
const css = `@import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@600;700;800&family=Barlow:wght@400;500;600&display=swap'); *{box-sizing:border-box;margin:0;padding:0;} body{background:#0A0A0A;color:#F0F0F0;font-family:'Barlow',sans-serif;} ::-webkit-scrollbar{width:6px;height:6px;} ::-webkit-scrollbar-track{background:#141414;} ::-webkit-scrollbar-thumb{background:#2ABFBF33;border-radius:3px;} ::-webkit-scrollbar-thumb:hover{background:#2ABFBF66;}`;
...
// --- PERSON DETAIL PANEL ------------------------------------------
function PersonPanel({ personId, token, onClose, onUpdated, t }) {
...
{opts.map(o=><option key={o} value={o}>{o==="All"?label:o}</option>)}
  ...
  {selectedId && (
    <PersonPanel personId={selectedId} token={token} onClose={()=>setSelectedId(null)} onUpdated={load} t={t} />
  )}
</div>
...
<span style={{marginRight:8}}>{GIFTING_ICONS[g]||"◆"}</span>{g}
  ...
  {selectedId && (
    <PersonPanel personId={selectedId} token={token} onClose={()=>setSelectedId(null)} onUpdated={()=>load(selectedGifting)} t={t} />
  )}
</div>
...
<style>{css}</style>
  ...
  <div style={{maxWidth:1400,margin:"0 auto"}}>
    {tab === "analytics" && <AnalyticsTab token={token} t={t} />}
    {tab === "people" && <PeopleTab token={token} t={t} />}
    {tab === "gifting" && <GiftingTab token={token} t={t} />}
  </div>
</div>
